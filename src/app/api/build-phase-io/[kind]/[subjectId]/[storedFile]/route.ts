...
function asciiFallbackFileName(name: string): string {
  ...
  return base.length > 0 ? base : "phase.io";
}
...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const [row] = await db
    .select({
      mimeType: buildReplicatePhases.ioMimeType,
      originalName: buildReplicatePhases.ioOriginalName,
    })
    .from(buildReplicatePhases)
    .where(
      and(
        eq(buildReplicatePhases.subjectKind, kind),
        eq(buildReplicatePhases.subjectId, subjectId),
        eq(buildReplicatePhases.ioStoredFile, storedFile)
      )
    )
    .limit(1);
