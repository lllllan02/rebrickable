...
export default async function PartDetailPage({ params }: Props) {
  ...
  const [asParent, asChild, elemRows, setRows] = await Promise.all([
    db
      .select({
      ...
      .orderBy(asc(partRelationships.relType), asc(partRelationships.childPartNum))
      .limit(200),
    db
      .select({
        relType: partRelationships.relType,
      ...
      .orderBy(asc(partRelationships.relType), asc(partRelationships.parentPartNum))
      .limit(200),
    db
      .select({
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <h1 className="mt-2 font-mono text-2xl font-semibold text-[var(--accent)]">
          {row.partNum}
        </h1>
