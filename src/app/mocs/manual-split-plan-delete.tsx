...
import { deleteManualSplitPlan } from "@/app/mocs/manual-split-actions";
...
export function ManualSplitPlanDeleteButton({
  subjectKind,
  subjectId,
  planId,
  planDisplayName,
  ...
  const onConfirm = useCallback(() => {
    ...
    startTransition(async () => {
      const r = await deleteManualSplitPlan({ subjectKind, subjectId, planId });
      if (!r.ok) {
        setError(r.error);
        return;
      }
  ...
  }, [onDeleted, planId, router, subjectId, subjectKind]);
  ...
  const confirmPanel = confirming ? (
    ...
    >
      ...
      <p className="mt-1 leading-relaxed text-red-200/85">
        将永久删除「{planDisplayName}」及其 {bagCount} 个分包，且无法恢复。
      </p>
